...
func EnableDevice(service *edge_devices.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Enable device
		device, err := service.EnableDevice(c.Request.Context(), *tenantID, deviceID)
		if err != nil {
			if errors.Is(err, edgeerrors.ErrDeviceNotFound) {
				c.JSON(http.StatusNotFound, gin.H{"success": false, "error": "Not found"})
				return
			}
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": "failed to enable device"})
			return
		}
