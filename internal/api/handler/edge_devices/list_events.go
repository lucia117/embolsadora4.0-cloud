...
// ListEvents retrieves the historical log of all check events for a device.
func ListEvents(service *edge_devices.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// List events
		events, err := service.ListEvents(c.Request.Context(), *tenantID, deviceID)
		if err != nil {
			if errors.Is(err, edgeerrors.ErrDeviceNotFound) {
				c.JSON(http.StatusNotFound, gin.H{"success": false, "error": "Not found"})
				return
			}
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": "failed to list events"})
			return
		}

		// Convert to response DTOs
		responses := make([]dto.DeviceEventResponse, len(events))
		for i, event := range events {
			responses[i] = dto.DeviceEventToResponse(event)
		}
