...
- [X] T013 [P] [US1] Crear `migrations/000001_initial_schema.down.sql` con el contenido `DROP SCHEMA public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO public;` (bloque del Paso 2 de `quickstart.md`).
...
**Goal**: Cargar roles, permisos, tenant MRG y usuario admin MRG (en `pending_invitation`) de forma idempotente, para que la app pueda autenticar y autorizar inmediatamente tras un deploy.

**Independent Test**: Sobre una DB con `000001` aplicado, ejecutar `migrate up 1` (que aplica `000002`); verificar `SELECT count(*) FROM roles WHERE is_system=true` ≥ 4, `SELECT count(*) FROM permissions` > 0, `SELECT count(*) FROM tenants WHERE is_platform=true` = 1, `SELECT count(*) FROM users WHERE status='pending_invitation'` ≥ 1, `SELECT count(*) FROM user_tenant_roles WHERE user_id=<admin>` = 1. Re-ejecutar `down 1; up 1` sin errores.
...
- [X] T019 [P] [US2] Crear `migrations/000002_seed_essentials.down.sql` con el bloque DELETE en orden inverso (FK-safe). Validar mentalmente que respeta las FKs: primero `user_tenant_roles WHERE tenant_id=<MRG> OR role_id IN (...)`, luego `tenants WHERE id=<MRG>`, luego `roles WHERE id IN (...)`, finalmente `permissions WHERE is_system_permission=true`. (Sin DELETE de `users` ni de `role_permissions` — esas filas no se siembran, ver T018 actualizada.)
