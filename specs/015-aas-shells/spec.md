...
> **Nota de rescate (2026-09-01)**: este spec y el resto de los archivos de
> `specs/015-aas-shells/` son una copia de `specs/014-mongo-infra/` tal como
> quedaron en el PR #30 (`006-mongo-infra`), cerrado sin mergear por drift
> de ~5 meses y conflictos con `develop`. La parte de infraestructura Mongo
> que describe este documento (conexión, cliente, wiring) ya está cubierta
> por PR #75 (endpoint de ingesta del cloud) con un diseño distinto y más
> simple — no hace falta retomarla. Lo que **no** se implementó en ningún
> otro lado es el CRUD de Asset Administration Shell / Submodel en sí
> (User Stories 2 y 3 más abajo): eso sigue pendiente si se retoma este
> feature. El código de referencia (con drift, no mergeable tal cual) queda
> en la rama `006-mongo-infra` / PR #30 cerrado.
>
> El `ADR-005 pendiente de formalizar` que menciona el párrafo de abajo
> **nunca se creó** — ni en esta rama ni en `develop`. Si se retoma este
> feature, ese ADR (justificar por qué Mongo y no Postgres/JSONB para los
> documentos AAS) sigue siendo un paso pendiente real, no solo una
> formalidad.
>
> Antes de reimplementar el endpoint de consumo (`GET
> /api/v1/consumers/shells/:id`), ojo: la versión original de PR #30 espera
> que el device mande `X-Tenant-Id`, lo que contradice el patrón fijado en
> PR #75 para `/api/v1/consumers/*` (el tenant se resuelve server-side a
> partir de la API key, el device nunca manda ese header). Habría que
> rediseñar ese endpoint para ser consistente.
